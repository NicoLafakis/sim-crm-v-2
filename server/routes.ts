...
Please provide a detailed simulation strategy with realistic business scenarios, persona profiles, and data generation recommendations in JSON format.`;
  ...
  app.get('/api/job/:jobId/status', async (req, res) => {
    try {
      ...
      const steps = await storage.getDueJobSteps(new Date());
      
      // Calculate step counts
      const totalSteps = steps.length;
      const completedSteps = steps.filter((step: any) => step.status === 'completed').length;
      const failedSteps = steps.filter((step: any) => step.status === 'failed' || step.status === 'failed_non_retryable').length;
      const queuedSteps = steps.filter((step: any) => step.status === 'queued').length;
      ...
      res.json({
        ...
        updatedAt: job.createdAt // Use createdAt since updatedAt doesn't exist
      });
  ...
  // Test endpoint to create a 10-step simulation job for testing progress
  app.post('/api/test/create-job', async (req, res) => {
    try {
      const { userId = 1, stepCount = 10 } = req.body;
      
      // Create a test simulation
      const simulation = await storage.createSimulation({
        userId,
        name: `Test Simulation (${stepCount} steps)`,
        theme: 'star_wars',
        industry: 'space_exploration',
        frequency: 'test',
        status: 'processing',
      ...
      const actionTypes = [
        ...
        'create_company_persona', 
        'create_deal_persona',
        'create_ticket_persona',
        'create_contact',
        'create_company',
        'create_deal',
        'create_ticket',
      ...
      for (let i = 0; i < stepCount; i++) {
        ...
        const step = await storage.createJobSteps([stepData]);
        steps.push(step[0]);
      }

      res.json({
        message: `Test job created with ${stepCount} steps`,
        simulationId: simulation.id,
        jobId: job.id,
